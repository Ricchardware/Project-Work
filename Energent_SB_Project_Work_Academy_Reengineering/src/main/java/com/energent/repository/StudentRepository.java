...
public interface StudentRepository extends JpaRepository<Student, String>{
	...
	@Modifying
	@Transactional
	@Query(value="SELECT DISTINCT `student_id` FROM `academy_student` WHERE academy_id NOT IN(?1) ",
	nativeQuery=true)
	List<String> outside( String code);
	
	
	@Modifying
	@Transactional
	@Query(value="SELECT * FROM `academy_student` WHERE academy_id=?1 AND student_id = ?2",
	nativeQuery=true)
	List<String> lookForSomeoneHere( String code, String fiscalCode);
